...
This module implements Pure Monte Carlo Game Search: MCTS with random selection in tree.
"""
...
class PMCGS(MCTSAlgorithm):
    """
    Pure Monte Carlo Game Search: MCTS with random selection in tree.
    """

    def select_move(self) -> int:
        """
        Run PMCGS algorithm and select best move.

        Returns:
            Selected column number (1-indexed)
        """
        for _ in range(self.num_simulations):
            # Selection: traverse tree randomly until leaf
            ...
            while not self.game.is_terminal()[0]:
                legal_moves = self.game.get_legal_moves()
                if len(legal_moves) == 0:
                    break
            ...
            if self.verbose:
                # Print updated values up the path (from leaf to root, excluding root)
                path = []
                n = node
                while n is not None:
                    path.append(n)
                    n = n.parent
                # path is now [leaf, parent, grandparent, ..., root]
                # We want to print from leaf to root (excluding root), so skip last element
                for n in path[:-1]:  # Skip root (last element)
                    print(f"Updated values:\nwi: {n.wi}\nni: {n.ni}\n")

        # Select best move based on average value
        best_move = None
